...
void notebook_t::remove(tree_t * src) {
	shell_surface * mw = dynamic_cast<shell_surface*>(src);
	if (has_key(_clients, mw) and mw != nullptr) {
		remove_client(mw);
	}
}

void notebook_t::activate_client(shell_surface * x) {
	if ((_client_map.find(x)) != _client_map.end()) {
		set_selected(x);
	}
}

std::list<shell_surface *> const & notebook_t::get_clients() {
	return _clients;
}
...
void notebook_t::set_selected(shell_surface * c) {
//	/** already selected **/
...
//		/** store current shell_surface then iconify **/
...
void notebook_t::compute_client_size_with_constraint(shell_surface * c,
...
i_rect notebook_t::compute_client_size(shell_surface * c) {
...
shell_surface const * notebook_t::selected() const {
	return _selected;
}
...
shell_surface * notebook_t::get_selected() {
	return _selected;
}
